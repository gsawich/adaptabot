...
class Poloniex:
    ...
    def api(self, type, params):
        ...
        if 'private' == type:
            ...
            print(post_data)
            sign = hmac.new(self.Secret.encode('ascii'), post_data, hashlib.sha512).hexdigest()
            headers = {
                'Sign': sign,
                'Key': self.APIKey
            }

            ret = urlopen(Request('https://poloniex.com/tradingApi', post_data, headers, method='POST'))
            jsonRet = json.loads(ret.read())
